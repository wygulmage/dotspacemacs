    ...
    '(:eval (when (and vc-mode buffer-file-name)
              (let ((branch (vc-working-revision buffer-file-name))
                ...
                (cond ((eq state 'up-to-date)
                       (propertize branch 'face '(:foreground "#99cc99")))
                      ((eq state 'edited)
                       (propertize branch 'face '(:foreground "#bbdaff")))
                      ((eq state 'needs-merge)
                       (propertize branch 'face '(:foreground "#ffc58f")))
                      ((eq state 'removed)
                       (propertize branch 'face '(:foreground "#ff9da4")))
                      (t branch)))))
