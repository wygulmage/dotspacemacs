...
values."
  (setq-default
   ...
   spacemacs-delete-orphan-packages t))
...
values."
  ...
  (setq-default
   ...
   ;; Which-key frame position. Possible values are `right', `bottom' and `right-then-bottom'. right-then-bottom tries to display the frame to the right; if there is insufficient space it displays it at the bottom.
   ;; (default 'bottom)
   ...
   '("ag" "pt" "ack" "grep")
   ;; The default package repository used if no explicit repository has been specified with an installed package.
...
(defun dotspacemacs/user-config ()
 ...
 This function is called at the very end of Spacemacs initialization after layers configuration. You are free to put any user code."
  ...
  (defun my-style-frame-title ()
    (setq frame-title-format (list my-buffer-or-file-name-string ;; file location
                                   " " my-buffer-modified-string)))
